...
pub fn get_next_queued(db: &DbPool, project_id: i64) -> Option<Task> {
    let conn = db.lock();
    let mut stmt = match conn.prepare("SELECT * FROM tasks WHERE project_id=?1 AND status='backlog' AND deleted_at IS NULL AND (task_level IS NULL OR task_level NOT IN ('epic','story')) AND (retry_after IS NULL OR retry_after <= datetime('now','localtime')) ORDER BY priority DESC,queue_position ASC,id ASC LIMIT 1") {
        Ok(s) => s,
