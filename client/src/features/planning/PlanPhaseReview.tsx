...
// `../board/DependencyGraph` is converted separately; narrow its (currently
...
interface PlanPhaseReviewProps {
  ...
  setExpandedTask: Dispatch<SetStateAction<number | null>>;
  showDag: boolean;
      ...
      <div>
        <div className="flex items-center justify-between mb-2.5">
          <label className="flex items-center gap-1.5 text-xs font-medium text-surface-300">
            <ListChecks size={13} className="text-amber-400" />
            {t('planning.proposedTasks').replace('{count}', String(proposals.length))}
          </label>
        ...
        <div className="space-y-1.5 max-h-80 overflow-y-auto pr-1">
          {proposals.map((task, i) => {
            ...
            const typeColor =
              TYPE_COLORS[task.task_type as keyof typeof TYPE_COLORS] || 'bg-surface-500/15 text-surface-400';
            return (
              <div
                key={i}
