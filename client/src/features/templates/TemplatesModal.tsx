...
function HighlightedTextarea({ value, onChange, placeholder, rows }: HighlightedTextareaProps) {
  return (
...
interface TemplateFormProps {
  ...
  onSave: (data: Partial<Template> & { taskType?: string; thinkingEffort?: string }) => void;
  onCancel: () => void;
}
...
function TemplateForm({ template, onSave, onCancel }: TemplateFormProps) {
  ...
  const [variables, setVariables] = useState<TemplateVariable[]>(() => {
    try {
      return template?.variables ? (JSON.parse(template.variables) as TemplateVariable[]) : [];
    } catch {
      return [];
    }
  });
...
export default function TemplatesModal({ projectId, projectName, onClose }: Props) {
            ...
            {crud.items.length > 0 && !crud.editing && (
              <div className="space-y-2 mb-4">
                {crud.items.map((t) => {
                  ...
                  try {
                    vars = JSON.parse(t.variables || '[]') as TemplateVariable[];
                  } catch {}
                  return (
