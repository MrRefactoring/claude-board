...
const CHECK_BG: Record<string, string> = {
...
interface AutoTestCheck {
  status: string;
  name: string;
  detail: string;
}

interface AutoTestVerdict {
  passed: boolean;
  summary: string;
}
...
function AutoTestCheckLine({ check }: { check: AutoTestCheck; time?: string }) {
...
function AutoTestVerdictLine({ verdict, time }: { verdict: AutoTestVerdict; time?: string }) {
...
export function SystemLine({ log }: { log: LogLine }) {
