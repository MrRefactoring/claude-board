...
// ─── Unified tool card ───
export function ToolCard({ call, result, isExpanded, onToggle }: ToolCardProps) {
  const meta: LogMeta = call?.meta || result?.meta || {};
  const toolName = meta.toolName || 'unknown';
  const Icon = getToolIcon(toolName);
  const color = getToolColor(toolName);
  const input: ToolInput = call?.meta?.input || {};
  const resMeta: LogMeta = result?.meta || {};
