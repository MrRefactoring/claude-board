...
function TaskPane({ task, logs }: { task: Task; logs: ProjectLogEntry[] }) {
...
export default function ProjectTerminal({ tasks }: { tasks?: Task[] }) {
  ...
  const [autoScroll, setAutoScroll] = useState(true);
  const containerRef = useRef<HTMLDivElement>(null);
  const pausedRef = useRef<ProjectLogEntry[]>([]);
  const pausedFlagRef = useRef(false);
  const tasksByIdRef = useRef<Record<number, Task>>({});
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((l) => l.taskId !== undefined && visibleTaskIds.has(l.taskId));
