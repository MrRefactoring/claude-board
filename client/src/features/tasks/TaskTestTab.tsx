...
export function TaskTestTab({ d }: Props) {
  const { t } = useTranslation();

  // Only the parse can throw — keep the try around it, never around JSX
  // (a try/catch does not catch child render errors anyway).
  let report: ReturnType<typeof parseTestReport>;
  try {
    report = parseTestReport(d.test_report);
  } catch {
