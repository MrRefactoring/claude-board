...
/** Task rows carry an optional `tags` column (JSON string or already-parsed array). */
type TaskWithTags = Task & { tags?: string | string[] };

/** The payload the modal emits — camelCase keys the create/update handlers accept. */
...
interface Props {
  ...
  onSubmit: (data: TaskSubmitData) => void | Promise<void>;
  onClose: () => void;
  templates?: Template[];
  roles?: Role[];
...
export default function TaskModal({ task, onSubmit, onClose, templates = [], roles = [], allTasks = [] }: Props) {
  const { t } = useTranslation();
  const { models: MODELS } = useModels();
  const [title, setTitle] = useState(task?.title || '');
  const [description, setDescription] = useState(task?.description || '');
  const [priority, setPriority] = useState(task?.priority || 0);
  const [taskType, setTaskType] = useState<string>(task?.task_type || 'feature');
  ...
  const [roleId, setRoleId] = useState<number | null>(task?.role_id || null);
  const [attachedFiles, setAttachedFiles] = useState<File[]>([]);
  const [selectedTemplate, setSelectedTemplate] = useState<Template | null>(null);
  const [templateVars, setTemplateVars] = useState<Record<string, string>>({});
  const [showOptions, setShowOptions] = useState(
    !!(task && (task.acceptance_criteria || task.role_id || (task.priority ?? 0) > 0)),
  );
  const [dependencies, setDependencies] = useState<TaskDependencies>({ parents: [], children: [] });
  const [tags, setTags] = useState<string[]>(() => {
    if (!task?.tags) return [];
    try {
      return typeof task.tags === 'string' ? (JSON.parse(task.tags) as string[]) : task.tags;
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((deps) => setDependencies(deps as TaskDependencies))
  ...
  const optionsSummary = useMemo(() => {
    const parts = [];
    const m = MODELS.find((x: Model) => x.value === model);
  ...
  const generatedDescription = useMemo(() => {
    ...
    try {
      vars = JSON.parse(selectedTemplate.variables || '[]') as TemplateVariable[];
    } catch {}
    for (const v of vars) {
      if (v.name) {
  ...
  const handleSelectTemplate = (tpl: Template) => {
    ...
    try {
      for (const v of JSON.parse(tpl.variables || '[]') as TemplateVariable[]) {
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-4 space-y-3">
            ...
            {showOptions && (
              <TaskOptionsPanel
                ...
                tagSuggestions={[
                  ...new Set(
                    allTasks.flatMap((t): string[] => {
                      try {
                        return typeof t.tags === 'string' ? (JSON.parse(t.tags) as string[]) : t.tags || [];
                      } catch {
                        return [];
                      }
                    }),
