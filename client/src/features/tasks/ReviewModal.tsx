...
interface Props {
  task: Task;
  onApprove: (taskId: number) => void | Promise<void>;
  onRequestChanges: (taskId: number, feedback: string) => void | Promise<void>;
  onClose: () => void;
}

export default function ReviewModal({ task, onApprove, onRequestChanges, onClose }: Props) {
  const { t } = useTranslation();
  const [feedback, setFeedback] = useState('');
  const [revisions, setRevisions] = useState<TaskRevision[]>([]);
  const [loading, setLoading] = useState(false);
  const [mode, setMode] = useState<'reject' | null>(null); // null = choose, 'reject' = writing feedback
  ...
  useEffect(() => {
    api
      ...
      .then(setRevisions)
      .catch((e: unknown) => console.error('Failed to load revisions:', e));
  }, [task.id]);
