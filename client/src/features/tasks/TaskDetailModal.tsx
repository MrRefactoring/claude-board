...
export default function TaskDetailModal({ task, onClose, onStatusChange }: Props) {
  ...
  useEffect(() => {
    api
      ...
      .then((raw) => {
        const detailData = raw as TaskDetail;
  ...
  const d = (detail || task) as TaskDetail;
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-surface-800 flex-shrink-0">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {showStatusMenu && (
                  <div className="absolute top-full left-0 mt-1 bg-surface-800 border border-surface-700 rounded-lg py-1 shadow-xl min-w-[140px] z-10">
                    {COLUMNS.filter((c) => c.id !== currentStatus).map((c) => (
                      <button
                        key={c.id}
                        onClick={() => handleStatusChange(c.id as TaskStatus)}
              ...
              <TagList tags={d.tags} max={5} size="sm" onTagClick={undefined} />
            </div>
