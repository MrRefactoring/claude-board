...
interface Props {
  ...
  onAdd: (taskId: number | string | undefined, depId: number) => void | Promise<void>;
  onRemove: (taskId: number | string | undefined, depId: number) => void;
}

export default function DependencySelector({ taskId, allTasks, dependencies, onAdd, onRemove }: Props) {
  ...
  const parentTasks = useMemo(
    () => parents.map((id) => allTasks.find((x) => x.id === id)).filter((x): x is Task => Boolean(x)),
    [parents, allTasks],
  );

  const childTasks = useMemo(
    () => children.map((id) => allTasks.find((x) => x.id === id)).filter((x): x is Task => Boolean(x)),
    [children, allTasks],
  );
  ...
  const handleAdd = async (depId: number) => {
    setCycleError(null);
    try {
