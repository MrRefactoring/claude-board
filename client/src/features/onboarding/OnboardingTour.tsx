...
// ─── Steps ───
function getSteps(t: TranslateFn, hasProject: boolean): Step[] {
  const steps: Step[] = [
    {
      id: 'welcome',
      position: 'center',
...
function getTooltipPos(targetEl: Element | null, position: string, tipWidth?: number): CSSProperties {
...
export default function OnboardingTour({ active, onComplete, hasProject }: OnboardingTourProps) {
  const { t } = useTranslation();
  const [step, setStep] = useState(0);
  const [tipStyle, setTipStyle] = useState<CSSProperties>({});
  const [hlStyle, setHlStyle] = useState<HighlightStyle | null>(null);
