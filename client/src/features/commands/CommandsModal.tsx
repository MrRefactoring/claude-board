...
export default function CommandsModal({ onClose }: Props) {
  const { t } = useTranslation();
  const [commands, setCommands] = useState<CustomCommand[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<CustomCommand | null>(null);

  useEffect(() => {
    api
      .listCustomCommands()
      .then((data) => setCommands(data as CustomCommand[]))
      .catch(() => setCommands([]))
