...
/** Activity row as returned to the timeline (metadata arrives pre-parsed). */
...
export default function ActivityTimeline({ projectId, onClose }: { projectId: number; onClose: () => void }) {
  ...
  const loadEvents = useCallback(
    async (offset = 0) => {
      try {
        const data = (await api.getActivity(projectId, 50, offset)) as unknown as ActivityEvent[];
        if (offset === 0) {
          setEvents(data);
        } else {
