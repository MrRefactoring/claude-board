...
interface DashboardProps {
  ...
  onNewProject: () => void;
  onOpenSettings?: () => void;
  onDeleteProject?: (project: ProjectSummary, onDeleted: () => void) => void;
}

interface DashHeaderProps {
  t: TranslateFn;
  dashTab: DashTab;
  setDashTab: Dispatch<SetStateAction<DashTab>>;
  onNewProject: () => void;
  onOpenSettings?: () => void;
}
...
let groupsCache: ProjectGroup[] | null = null;
let suggestionsCache: Suggestion[] | null = null;
let suggestionsLoaded = false;

function DashHeader({ t, dashTab, setDashTab, onNewProject, onOpenSettings }: DashHeaderProps) {
  return (
    <div
...
export default function Dashboard({
  ...
  const [summary, setSummary] = useState<ProjectSummary[]>(summaryCache || []);
  const [groups, setGroups] = useState<ProjectGroup[]>(groupsCache || []);
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      const v = !groupBy;
                      setGroupBy(v);
                      localStorage.setItem('dashboard:groupBy', String(v));
                    }}
...
function StatCard({ icon: Icon, label, value, color, glow, pulse }: StatCardProps) {
  return (
    <div
