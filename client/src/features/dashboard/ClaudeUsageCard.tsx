...
interface ClaudeUsageTimelinePoint {
  day?: string;
  tokens?: number;
  cost?: number;
}
...
interface ClaudeUsageData {
  ...
  timeline?: ClaudeUsageTimelinePoint[];
  limits?: ClaudeUsageLimits | null;
}

// Cache outside component so it survives remounts — shared with Dashboard
export let usageCache: ClaudeUsageData | null = null;
export function setUsageCache(val: ClaudeUsageData | null) {
  usageCache = val;
}
            ...
            <div>
              ...
              <div className="space-y-2">
                {models.map((m) => {
                  ...
                  const colorClass = MODEL_COLORS[displayName as keyof typeof MODEL_COLORS] || MODEL_COLORS.unknown;
                  return (
              ...
              <div className="flex items-end gap-px h-16">
                {timeline.slice(-30).map((d, i) => (
                  <div
                    ...
                    style={{ height: `${Math.max(2, ((d.tokens || 0) / maxTokens) * 100)}%` }}
        ...
        {(u.cache_read || 0) > 0 && (
          <div className="mt-4 pt-3 border-t border-surface-700/30 flex items-center gap-4 text-[10px] text-surface-500">
            ...
            <span className="text-emerald-500/70">
              {totalTokens > 0
                ? `${(((u.cache_read || 0) / (totalTokens + (u.cache_read || 0))) * 100).toFixed(0)}% ${t('usage.cacheHit')}`
                : ''}
            </span>
          </div>
        )}
