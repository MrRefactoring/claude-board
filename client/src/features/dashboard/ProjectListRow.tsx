...
export function ProjectListRow({ project, onSelect, onDelete, t }: ProjectListRowProps) {
  const total = project.total_tasks || 0;
  const variant = (project.icon || 'marble') as ComponentProps<typeof Avatar>['variant'];
    ...
    >
      ...
      {total > 0 && (
        <div className="flex items-center gap-3 text-[10px] text-surface-500 flex-shrink-0">
          ...
          {(project.total_cost || 0) > 0 && <span>${(project.total_cost || 0).toFixed(2)}</span>}
        </div>
      )}
      {total > 0 && (
