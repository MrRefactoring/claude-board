...
function BarChart({ data, colorMap, maxVal }: BarChartProps) {
...
export default function AnalyticsView({ tasks, projectId }: AnalyticsViewProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const tasksWithCost = tasks.filter((t) => (t.total_cost ?? 0) > 0);
    ...
    const modelMap: Record<string, ModelDatum> = {};
    tasks.forEach((task) => {
    ...
    const completedTasks = tasks
      .filter((t) => t.started_at && ((t.total_cost ?? 0) > 0 || (t.input_tokens ?? 0) > 0))
      .map((t) => ({
        ...t,
  ...
  const costTimeline = useMemo(() => {
    ...
    const dayMap: Record<string, CostDatum> = {};
