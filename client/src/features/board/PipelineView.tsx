...
type EnrichedPipelineTask = PipelineTask & { _parentIds: number[] };
...
export default function PipelineView({ tasks, onViewLogs, onViewDetail }: PipelineViewProps) {
  const { t } = useTranslation();
  const [depMap, setDepMap] = useState<Record<number, number[]>>({});
  ...
  useEffect(() => {
    ...
    const loadDeps = async () => {
      ...
      for (const task of tasks) {
        try {
          const deps = (await api.getTaskDependencies(task.id)) as TaskDeps;
          if (deps.parents && deps.parents.length > 0) map[task.id] = deps.parents;
        } catch {}
      }
      setDepMap(map);
      setLoading(false);
  ...
  const [localQueue, setLocalQueue] = useState<EnrichedPipelineTask[] | null>(null);
  ...
  const avgDuration = (() => {
    const durs = completed.filter((t) => (t.work_duration_ms ?? 0) > 0).map((t) => t.work_duration_ms ?? 0);
  ...
  const handleQueueDrop = (targetIdx: number) => {
...
function Section({ title, count, color, children, collapsible }: SectionProps) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {(task.work_duration_ms ?? 0) > 0 && (
            <span className="text-[9px] text-surface-600">{Math.round((task.work_duration_ms ?? 0) / 1000)}s</span>
          )}
          {(task.total_cost ?? 0) > 0 && (
            <span className="text-[9px] text-surface-600">${(task.total_cost ?? 0).toFixed(3)}</span>
          )}
          {depIds.length > 0 ? (
