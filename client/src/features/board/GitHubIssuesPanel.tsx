...
function getLabelClass(name: string): string {
...
export default function GitHubIssuesPanel({ projectId, onClose }: GitHubIssuesPanelProps) {
  ...
  const [importedNow, setImportedNow] = useState<Set<number>>(new Set());
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      setError((e as Error).message || 'Failed to fetch issues');
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setError((e as Error).message || 'Failed to import issues');
    }
    setImporting(false);
  };
