...
interface BattleTask extends Task {
  agent_name?: string;
}

interface ArenaAgent extends BattleTask {
  _pos: { x: number; y: number };
  _state: 'active' | 'victory' | 'defeat';
}
...
function Projectile({ fromX, fromY, toX, toY, size, attack, onDone }: ProjectileProps) {
      ...
      {particles.map((p) => {
        ...
        return (
          <div
            ...
            style={{ '--tx': `${tx}px`, '--ty': `${ty}px`, animationDelay: `${p.delay}s` } as CSSProperties}
          >
            <div
...
function DmgNumber({ value, x, y, color, isCrit }: DmgNumberProps) {
  return (
    <div
...
function AgentSprite({ task, x, y, hp, maxHp, isHit, lastAction, isVictory, isDefeat }: AgentSpriteProps) {
...
export default function BattleView({ tasks, projectId }: BattleViewProps) {
  ...
  const [hpMap, setHpMap] = useState<Record<number, number>>({});
  ...
  useEffect(() => {
    ...
    const load = () =>
      api
        ...
        .then((d) => setConflicts((d as AgentActivityPayload | undefined)?.conflicts || []))
        .catch(() => {});
    void load();
  ...
  const removeProjectile = useCallback((id: number) => setProjectiles((prev) => prev.filter((p) => p.id !== id)), []);

  // Victory/defeat
  useEffect(() => {
    if (!IS_TAURI) return;
    return tauriListen('task:updated', (payload) => {
      const p = payload as Partial<Task> & { id?: number; taskId?: number };
      const tid = (p.id ?? p.taskId) as number;
      if (p.status === 'done' || p.status === 'testing') {
