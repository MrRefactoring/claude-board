...
interface AgentCardProps {
  ...
  onViewLogs?: (task: AgentTask) => void;
}
...
export default function AgentCard({ task, onStop, onViewLogs }: AgentCardProps) {
  ...
  const [liveUsage, setLiveUsage] = useState<{ input: number; output: number; cost: number }>({
  ...
  const [lastTool, setLastTool] = useState<{ name: string; path: string } | null>(null);
  ...
  useEffect(() => {
    if (!IS_TAURI) return;
    return tauriListen('task:log', (raw) => {
