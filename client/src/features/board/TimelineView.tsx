...
const STATUS_COLORS: Record<string, { bar: string; barLight: string; text: string }> = {
...
function computeTimeScale(tasks: Task[], containerWidth: number): TimeScale | null {
  ...
  const starts = withTime.map((t) => new Date(t.started_at as string).getTime());
...
function formatTick(ts: number, intervalMs: number): string {
  const d = new Date(ts);
...
export default function TimelineView({ tasks, waves, edges, onTaskClick }: TimelineViewProps) {
  ...
  const { rows, waveLabels } = useMemo(() => {
    ...
    const waveLabels: { label: string; y: number }[] = [];
    let y = 0;
