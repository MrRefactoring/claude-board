...
interface ListViewProps {
  ...
  onViewLogs: (task: ListTask) => void;
  onEditTask: (task: ListTask) => void;
  onDeleteTask: (task: ListTask) => void;
  onBulkDelete?: (tasks: ListTask[]) => void;
  onReviewTask?: (task: ListTask) => void;
  onViewDetail?: (task: ListTask) => void;
}
...
export default function ListView({
  ...
  const toggleSelect = useCallback((taskId: number, e: MouseEvent) => {
    e.stopPropagation();
    setSelectedIds((prev) => {
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <span className="flex items-center gap-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[task.status ?? 'backlog']}`} />
                      <span className="text-surface-300">{t('status.' + task.status)}</span>
                    </span>
                  </td>
                  <td className="px-3 py-2 text-surface-400">
                    {(task.priority ?? 0) > 0 ? PRIORITY_LABELS[task.priority ?? 0] : '-'}
                  </td>
                  <td
                    className={`px-3 py-2 ${MODEL_COLORS[modelDisplay as keyof typeof MODEL_COLORS] || 'text-surface-400'}`}
                  >
                    {modelDisplay}
                  </td>
                  <td className="px-3 py-2 text-surface-400">{totalTokens > 0 ? formatTokens(totalTokens) : '-'}</td>
                  <td className="px-3 py-2 text-surface-400">
                    {(task.total_cost ?? 0) > 0 ? `$${(task.total_cost ?? 0).toFixed(4)}` : '-'}
                  </td>
