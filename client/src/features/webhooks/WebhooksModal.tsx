...
// the runtime shape the component actually works with.
...
interface TestResultState extends TestResult {
  id: number;
}
...
interface WebhookFormProps {
  ...
  onSave: (data: Partial<WebhookRow>) => void;
  onCancel: () => void;
}
...
export default function WebhooksModal({ projectId, projectName, onClose }: Props) {
  ...
  const handleTest = async (webhook: WebhookRow) => {
    ...
    setTestResult(null);
    try {
      const result = (await api.testWebhook(webhook.id)) as TestResult;
      setTestResult({ id: webhook.id, ...result });
    } catch (err) {
      setTestResult({ id: webhook.id, ok: false, error: (err as Error).message });
    }
    setTesting(null);
  };

  const getPlatform = (id?: string) => PLATFORMS.find((p) => p.id === id) || PLATFORMS[3];
